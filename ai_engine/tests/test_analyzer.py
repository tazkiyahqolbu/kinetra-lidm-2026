...
def test_analyzer_wires_up_all_engines(mock_pose_detector):
    ...
    assert isinstance(analyzer.filter, EMAFilter)
    assert isinstance(analyzer.state_machine, SquatStateMachine)
    assert isinstance(analyzer.score_engine, MovementScore)
    assert isinstance(analyzer.feedback_engine, FeedbackEngine)
    assert isinstance(analyzer.risk_engine, RiskDetection)
